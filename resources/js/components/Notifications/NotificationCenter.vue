<template>
  ...
  <teleport to="#notifications">
    <div v-if="notificationsShown" class="fixed flex inset-0">
      ...
      <div
        class="relative divide-y divide-gray-200 dark:divide-gray-700 shadow bg-white dark:bg-gray-800 w-[20rem] ml-auto border-b border-gray-200 dark:border-gray-700 overflow-x-hidden overflow-y-scroll"
      >
        <nav
        ...
        >
          <component
            :is="notification.data.component || `MessageNotification`"
            v-for="notification in notifications"
            :key="notification.id"
            :notification="notification.data"
            @mark-as-read="() => markNotificationAsRead(notification.id)"
            class="flex items-start py-6"
          />
        </div>
